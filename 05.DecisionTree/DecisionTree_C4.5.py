...
class ID3:
    ...
    def build(self, dataset, node):
        ...
        # Case2: 特征为空，将数据集中实例数最大的类作为该节点的标记
            ...
            node.data = dataset
        # Case3: 计算各特征的信息增益比，选择信息增益比最大的特征
        else:
            entropy = self.cal_entropy(dataset["label"])
            columns = X_train.columns
            gains = []
            for col in columns:
                info_gain = entropy - self.cal_conditional_entropy(dataset, col) # 信息增益
                #print(col, dataset[col])
                info_gain_ratio = info_gain / self.cal_entropy(dataset[col])
                gains.append(info_gain_ratio)
            max_gain = max(gains)
